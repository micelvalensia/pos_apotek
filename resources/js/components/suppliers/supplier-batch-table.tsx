...
import type { PaginatedData, SupplierBatch } from '@/types';

interface SupplierBatchTableProps {
    batches: PaginatedData<SupplierBatch>;
}

export function SupplierBatchTable({ batches }: SupplierBatchTableProps) {
    ...
    return (
        <div className="space-y-4">
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="bg-slate-50/75 dark:bg-neutral-800/50">
                            ...
                            <TableHead className="min-w-[180px] font-bold text-slate-700 dark:text-neutral-300">Nama Produk Obat</TableHead>
                            <TableHead className="min-w-[140px] font-bold text-slate-700 dark:text-neutral-300">No. Batch</TableHead>
                            <TableHead className="min-w-[130px] font-bold text-slate-700 dark:text-neutral-300">Kedaluwarsa</TableHead>
                            ...
                            <TableHead className="min-w-[140px] text-right font-bold text-slate-700 dark:text-neutral-300">Harga Beli Dasar</TableHead>
                            <TableHead className="min-w-[140px] text-right font-bold text-slate-700 dark:text-neutral-300">Total Nilai Sisa</TableHead>
                        </TableRow>
                    </TableHeader>
                    ...
                    <TableBody>
                        {batches.data.length > 0 ? (
                            batches.data.map((batch, index) => {
                                const rowNumber = (batches.current_page - 1) * batches.per_page + (index + 1);
                                        ...
                                        <TableCell>
                                            <div className="flex items-center gap-2.5">
                                                ...
                                                <span className="font-bold text-slate-900 dark:text-white text-xs">
                                                    {batch.product?.name ?? 'Produk Dihapus'}
                                                </span>
                                            </div>
                                        </TableCell>
                                ...
                                <TableCell colSpan={8} className="h-36 text-center">
                                    <div className="flex flex-col items-center justify-center text-slate-400 dark:text-neutral-500">
                                        ...
                                        <p className="text-xs font-semibold text-slate-600 dark:text-neutral-300">
                                            Belum Ada Riwayat Pasokan Obat
                                        </p>
                                        <p className="text-[11px] text-slate-400 mt-0.5">
                                            Supplier ini belum memiliki catatan penerimaan batch obat masuk.
                                        </p>
                                    </div>
                                </TableCell>
                            </TableRow>
            ...
            <DataPagination data={batches} itemName="batch pasokan" />
        </div>
    );
}
